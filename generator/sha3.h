...
/* 'Words' here refers to u_int64_t */
#define SHA3_KECCAK_SPONGE_WORDS \
	(((1600)/8/*bits to byte*/)/sizeof(u_int64_t))
typedef struct sha3_context_ {
    u_int64_t saved;             /* the portion of the input message that we
                                 * didn't consume yet */
    union {                     /* Keccak's state */
        u_int64_t s[SHA3_KECCAK_SPONGE_WORDS];
        u_int8_t sb[SHA3_KECCAK_SPONGE_WORDS * 8];
    };
