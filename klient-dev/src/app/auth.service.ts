...
export class AuthService {
  ...
  async login(username, password) {
    ...
    let jsondata;
    console.log("here")
    console.log('http://'+this.ipc.config.server+'/add-token')
    await new Promise((resolve, reject) => {
      this.http.post('http://'+this.ipc.config.server+'/add-token', { "username": username, client: true, "password": password })
        .toPromise()
        .then(
          (res) => {
            console.log(res)
  ...
  async loginWithToken(token, initial = false) {
    ...
    console.log("here")
    await new Promise((resolve, reject) => {
      this.http.post('http://'+this.ipc.config.server+'/add-token', { "key": token, client: true })
        .toPromise()
        .then(
          (res) => {
            console.log(res)
